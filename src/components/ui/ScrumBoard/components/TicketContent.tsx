...
const TicketContent = ({ onTicketClose }: { onTicketClose: () => void }) => {
    ...
    const transformCommentsToTicket = (
        ticketData: Partial<
            Omit<Ticket, 'comments'> & { comments: TransformedComment[] }
        >,
    ): Ticket => {
        const transformedComments = ticketData.comments?.map((comment) => ({
            ...comment,
            date: comment.date.getTime(), // Convert Date to number (timestamp)
        })) as Comment[]
            ...
            if (Object.hasOwnProperty.call(data, key)) {
                ...
                board.forEach((ticket, index) => {
                    if (ticket.id === ticketId) {
                        // Transform ticketData to match the Ticket type
                        data[key][index] = transformCommentsToTicket(ticketData)
                    }
                })
            }
        }
